    ...
    <div class="split-layout">
      ...
      <div class="panel-section left-panel">
        ...
        <div class="scroll-container">
          ...
          <div class="cards-list">
            <div v-for="driver in myDrivers" :key="driver.id" class="driver-card my-driver-card">
      ...
      <div class="panel-section right-panel">
        ...
        <div class="search-bar-container">
          <input type="text" v-model="filterText" placeholder="🔍 Buscar por nombre o teléfono..." class="search-input"
            :disabled="isLoading" />
        </div>
        ...
        <div class="scroll-container">
          ...
          <div v-else class="cards-list">
            <div v-for="driver in filteredHireableDrivers" :key="driver.id" class="driver-card hireable-card">
              <div class="driver-header">
              ...
              <div class="driver-actions">
                <!-- Se pasa el evento $event para manejar el estado del botón -->
                <button class="btn-hire" @click="hireDriver(driver, $event)">Contratar</button>
              </div>
            </div>
          </div>
        </div>
            ...
            <div class="modal-card" ref="modalCard" tabindex="-1">
              ...
              <div class="modal-body">
                <div class="form-group">
                  ...
                  <input type="text" v-model="searchPhone" @input="searchByPhone" class="input-field"
                    placeholder="Ej: 300123..." />
                </div>
              ...
              <footer class="modal-footer">
                <button class="btn-save" @click="saveDriver">Guardar y Vincular</button>
...
export default {
  ...
  data() {
    return {
      ...
      // Mis conductores (Datos locales de ejemplo inicial)
      myDrivers: [
  ...
  methods: {
    // --- LÓGICA CENTRAL DE VINCULACIÓN ---
    async linkDriverToCompany(phoneNumber) {
      const token = localStorage.getItem('token');
      if (!token) throw new Error("No hay sesión activa. Inicia sesión nuevamente.");

      // Endpoint y payload
      const url = 'https://service.lujuria.crudzaso.com/api/Drivers/link';
      ...
      if (!response.ok) {
        ...
        // Si el errorText es HTML (p.ej. página de error del servidor), mostramos un mensaje genérico
        if (errorText.includes("<!DOCTYPE html>")) {
          throw new Error(`Error ${response.status}: El servidor no respondió correctamente.`);
        }
        throw new Error(errorText || "Error al vincular el conductor.");
      }
      
      return true; // Éxito
    },

    // --- OBTENER CONDUCTORES (GET) ---
    async fetchDrivers() {
      this.isLoading = true;
      this.errorMsg = null;
      ...
      try {
        ...
        if (response.status === 401) throw new Error("Sesión expirada (401).");
        if (!response.ok) throw new Error(`Error ${response.status}: No se pudo obtener la información.`);

        const data = await response.json();

        // Mapeo de datos para normalizar mayúsculas/minúsculas
        const apiDrivers = data.map(d => ({
          id: d.id,
          name: d.name || d.Name || d.username || "Sin nombre",
          phone: d.phone || d.Phone || d.phoneNumber || "Sin teléfono",
          description: d.description || d.Description || "Conductor registrado"
        }));

        // Filtrar los que ya están en "Mis Conductores"
    ...
    async hireDriver(driver, event) {
      ...
      try {
        // 1. Intentar vincular en backend
        await this.linkDriverToCompany(driver.phone);

        // 2. Si tiene éxito, mover a la lista local
        this.myDrivers.push(driver);
        this.hireableDrivers = this.hireableDrivers.filter(d => d.id !== driver.id);
        this.filterText = "";

      } catch (error) {
        alert("⚠️ No se pudo contratar: " + error.message);
      } finally {
        // Restaurar botón
        if (btn) {
    ...
    async saveDriver() {
      ...
      if(btnSave) { btnSave.innerText = "Guardando..."; btnSave.disabled = true; }

      try {
        // Intentamos vincular el número ingresado
        await this.linkDriverToCompany(this.form.phone.trim());

        // Si éxito, agregamos a local
        this.myDrivers.push({
          id: this.searchResult ? this.searchResult.id : Date.now(),
        ...
        alert("⚠️ Error al vincular: " + error.message);
      } finally {
        if(btnSave) { btnSave.innerText = "Guardar y Vincular"; btnSave.disabled = false; }
      }
    },

    // --- ACCIÓN: DESVINCULAR (LOCAL) ---
    fireDriver(driver) {
      // Nota: Si existiera un endpoint DELETE /api/Drivers/unlink, se llamaría aquí.
    ...
    searchByPhone() {
      ...
      // Busca localmente en los que trajimos de la API
...
.btn-hire {
  width: 100%;
  padding: 8px;
  background: linear-gradient(90deg, #d4af37, #b78f2b);
  border: none;
  border-radius: 6px;
  color: #111;
  font-weight: bold;
