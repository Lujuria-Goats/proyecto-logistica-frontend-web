...
4. **Verificar URL del API:**
   ...
   - Verifica que `API_BASE_URL` apunta al backend correcto:
     ```javascript
     const API_BASE_URL = "http://service.apexvision.crudzaso.com/api";
     ```
...
- **API_BASE_URL:** `http://service.apexvision.crudzaso.com/api` (en `authApi.js`)
...
```
VITE_API_BASE_URL=http://service.apexvision.crudzaso.com/api
